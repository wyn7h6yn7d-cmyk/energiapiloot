...
export function CashflowChart({ data }: { data: CashflowPoint[] }) {
  ...
  return (
    <div className="ep-sim-chart-frame h-[17.5rem] w-full min-w-0 p-4">
      {mounted ? (
        ...
        <div className="h-full w-full rounded-2xl border border-[oklch(1_0_0_/_10%)] bg-[oklch(0_0_0_/_18%)]" aria-hidden />
      )}
    </div>
  );
}
              ...
              content={({ active, payload, label }) => {
                ...
                return tooltipBox(
                  <>
                    ...
                    <p className="mt-1 text-foreground/60">Tasuvus: {pb === null ? "—" : `${pb.toFixed(1)} a`}</p>
                  </>
                );
              }}
            />
